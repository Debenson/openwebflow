...
import org.openwebflow.cfg.ProcessEngineConfigurationEx;
import org.openwebflow.cfg.ReplaceTaskAssignmentHandler;
...
public abstract class AbstractProcessEngineTest {
  ...
  public void setUp() throws Exception {
    ...
    // 用户关系管理
    IdentityMembershipManagerEx myMembershipManager = _ctx
        .getBean(IdentityMembershipManagerEx.class);
    ...
    userDetailsStore.saveUserDetails(
        new SimpleUserDetailsEntity("bluejoe", "白乔", "bluejoe2008@gmail.com", "13800138000"));
    userDetailsStore.saveUserDetails(
        new SimpleUserDetailsEntity("kermit", "老黄", "heiker@trojo.com", "13800138000"));
    ...
    // 部署该model
    if (repositoryService.createProcessDefinitionQuery().processDefinitionKey("test1").list()
        .isEmpty()) {
    ...
    // 部署该model
    if (repositoryService.createProcessDefinitionQuery().processDefinitionKey("test2").list()
        .isEmpty()) {
  ...
  public void testInsertTasksBefore() throws Exception {
    ProcessInstance instance = _processEngine.getRuntimeService()
        .startProcessInstanceByKey("test2");
  ...
  public void testInsertTasksAfter() throws Exception {
    ...
    taskService.complete(taskService.createTaskQuery().singleResult().getId());
    // 应该到了下一个节点
    Assert.assertEquals(as[2].getId(),
        taskService.createTaskQuery().singleResult().getTaskDefinitionKey());
    Assert.assertEquals("alex", taskService.createTaskQuery().singleResult().getAssignee());
    // 完成该节点
    taskService.complete(taskService.createTaskQuery().singleResult().getId());
  ...
  public void testInsertTasksWithPersistence() throws Exception {
    ...
    // 重新构造以上对象
    instance = _processEngine.getRuntimeService().createProcessInstanceQuery()
  ...
  public void testSplit() throws Exception {
    ...
    int size = ProcessDefinitionUtils.getProcessDefinition(_processEngine, processDefId)
        .getActivities().size();
    ...
    Assert.assertEquals(size + 2, ProcessDefinitionUtils
  ...
  public void testCachedDefinitions() {
    ...
    ActivityImpl activity1 = ProcessDefinitionUtils.getActivity(_processEngine, processDefId,
        "step2");
    ActivityImpl activity2 = ProcessDefinitionUtils.getActivity(_processEngine, processDefId,
        "step2");
    Assert.assertSame(activity1, activity2);

    ExecutionEntity instance1 = (ExecutionEntity) _processEngine.getRuntimeService()
        .startProcessInstanceByKey("test2");
    ExecutionEntity instance2 = (ExecutionEntity) _processEngine.getRuntimeService()
        .startProcessInstanceByKey("test2");

    Assert.assertSame(instance1.getProcessDefinition(), instance2.getProcessDefinition());
    Assert.assertSame(
        ((ProcessDefinitionEntity) instance1.getProcessDefinition()).getTaskDefinitions(),
        ((ProcessDefinitionEntity) instance2.getProcessDefinition()).getTaskDefinitions());

    Assert.assertTrue(((ProcessDefinitionEntity) instance1.getProcessDefinition())
        .getTaskDefinitions().containsKey("step2"));
    Assert.assertTrue(((ProcessDefinitionEntity) instance1.getProcessDefinition())
        .getTaskDefinitions().containsKey("step3"));
    Assert.assertTrue(((ProcessDefinitionEntity) instance1.getProcessDefinition())
        .getTaskDefinitions().containsKey("step4"));
    Assert.assertTrue(((ProcessDefinitionEntity) instance1.getProcessDefinition())
        .getTaskDefinitions().containsKey("step5"));
    Assert.assertTrue(((ProcessDefinitionEntity) instance1.getProcessDefinition())
        .getTaskDefinitions().containsKey("step6"));
  ...
  public void testMultiInstancesLoop() {
    ...
    variables.put("assigneeList", CollectionUtils.arrayToList(new String[] {
        "kermit", "bluejoe" }));
    ...
    Assert.assertEquals(1, _processEngine.getRuntimeService().createExecutionQuery().count());
    Assert.assertEquals("step3",
        _processEngine.getRuntimeService().createExecutionQuery().singleResult().getActivityId());
    ...
    Assert.assertEquals(execution111.getParentId(), execution112.getParentId(),
        execution11.getId());
    ...
    Assert.assertEquals(task42.getId(),
        taskService.createTaskQuery().taskDefinitionKey("step4").singleResult().getId());
    ...
    Assert.assertEquals(
        taskService.createTaskQuery().taskDefinitionKey("step6").singleResult().getExecutionId(),
  ...
  public void testModelDeployment() throws Exception {
    ...
    // 由于没有部署，应该拿不到
    Assert.assertEquals(0, repositoryService.createProcessDefinitionQuery()
        .processDefinitionKey("vacationRequest").list().size());

    // 部署该model
    Deployment dep = ModelUtils.deployModel(repositoryService, model.getId());
    // 现在应该拿得到了
    Assert.assertEquals(1, repositoryService.createProcessDefinitionQuery()
        .processDefinitionKey("vacationRequest").list().size());
  ...
  public void testActivityPermission() throws Exception {
    ...
    _permissions.save(processDefId, "step2", null, new String[] {
        "engineering" }, new String[0]);
  ...
  public void testDelegation() throws Exception {
    ...
    ((TaskDelagationAssignmentHandler) (((ReplaceTaskAssignmentHandler) (((ProcessEngineConfigurationEx) _processEngine
        .getProcessEngineConfiguration()).getStartEngineEventListeners().get(2))).getHandlers()
  ...
  public void testAlarm() throws Exception {
    ...
    TaskAlarmServiceImpl taskAlarmService = (TaskAlarmServiceImpl) _ctx
        .getBean(TaskAlarmServiceImpl.class);
    ...
    Assert.assertEquals(taskId, taskIds.get(0));
